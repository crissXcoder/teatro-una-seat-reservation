...
export function SeatItem({ seat, isSelected, isSuggested, onClick }: SeatItemProps) {
  ...
  return (
    <button
      ...
      title={
        seat.estado 
          ? `Butaca ${seat.label} (Ocupada)` 
          : isSelected 
            ? `Butaca ${seat.label} (Seleccionada)` 
            : isSuggested 
              ? `Butaca ${seat.label} (Sugerida)` 
              : `Butaca ${seat.label} (Disponible)`
      }
      className={cn(
    ...
    >
      ...
      <div 
        className={cn(
          ...
          seat.estado && "bg-[#334155] border-slate-600 opacity-40 cursor-not-allowed", // Mejor contraste sobre fondo oscuro
      ...
      <div className={cn(
        ...
        seat.estado ? "bg-slate-800 opacity-30" : "bg-slate-500 opacity-70 group-hover:opacity-90"
      )}></div>
      <div className={cn(
